...
def test_get_fetcher_types():
    ...
    assert isinstance(f_eq, YFinanceFetcher.__class__) or isinstance(f_eq, YFinanceFetcher)
    assert isinstance(f_cr, OKXFetcher.__class__) or isinstance(f_cr, OKXFetcher)

@pytest.mark.skipif(os.getenv("ENABLE_NETWORK_TESTS","0") != "1", reason="network tests disabled by default")
@pytest.mark.asyncio
